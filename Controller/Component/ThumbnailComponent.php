...
class ThumbnailComponent extends Component {
  ...
  public function render($image, $params, $options = array()) {
    ...
    if (file_exists($this->settings['folder'] . '/' . $this->settings['cachePath'] . '/' . $this->settings['basename'])) {
  ...
  private function setup($image, $params) {
    ...
    $this->settings['cachePath'] = $this->settings['width'] . 'x' . $this->settings['height'] . '/' . $this->settings['cachePath'];
  }
